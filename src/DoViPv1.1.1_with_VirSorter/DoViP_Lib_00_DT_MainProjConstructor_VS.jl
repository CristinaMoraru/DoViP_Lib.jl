...
function set_predictors(dosteps::Dict{String, WorkflowStatus}) 
    ...
    for (k, v) in dosteps
        ...
        elseif k == "virSorter"
            if v.signal == "use" || v.signal == "use_external"
                push!(predictors_all, :predictor_virSorter)
            end
        elseif k == "vibrant"
            if v.signal == "use" || v.signal == "do" || v.signal == "use_external"
...
# this functions sets the ProjVirSorter struct, which accepts only external results atm
function set_ProjVirSorter(pd::String, sampleName::String, args::Vector{String}, signal::String)
    sp = "01f_ALL"
    virSorter_D = "$sampleName/$(sp)_virSorter"
    my_mkpath(["$(pd)/$(virSorter_D)"])

    if signal == "use_external"
        ext_res = true
        ext_res_D = extract_inPaths(args, "virSorter_res")
        out_vs_D = ""
    else
        ext_res = false
        ext_res_D = nothing
        out_vs_D = "$(virSorter_D)/$(sp)_01_virSorter_out"
    end

    vs_global_phage_sig = "$out_vs_D/VIRSorter_global-phage-signal.csv" |> TableP
    vs_prot_metrics = "$out_vs_D/Metric_files/VIRSorter_affi-contigs.tab"

    post_vs_nonintegrated_df = "$virSorter_D/post_VS_nonintegrated.tsv" |> TableP
    post_vs_integrated_df = "$virSorter_D/post_VS_integrated.tsv" |> TableP

    proj = ProjVirSorter(virSorter_D, ext_res, ext_res_D, vs_global_phage_sig, vs_prot_metrics, post_vs_nonintegrated_df, post_vs_integrated_df)
...
function ProjSViP_fun(args::Vector{String})
    #user
    user = extract_args(args, "user", "user"; allowed = ("user", "CristinaM", ""))
    ...
    if cont == false
        ...
        dosteps = Dict(
            ...
            "virSorter" => WorkflowStatus(
                setsignal(extract_args(args, "virSorter_signal", ("use_external", "dont", "use", "ignore", "remove")), "virSorter"),
                 "not_done"),
        ...
        if (dosteps["genomad"].signal == "dont" &&  dosteps["DVF"].signal == "dont"
            && dosteps["virSorter2"].signal == "dont" && dosteps["virSorter"].signal == "dont" && dosteps["vibrant"].signal == "dont"
        ...
        if user == "CristinaM"
            ...
            dosteps["DVF"].signal = setsignal(extract_args(args, "DVF_signal",("do", "dont", "use", "ignore", "remove")),
                                "DVF"; cont = true, 
                                oldsignal = prev_dosteps["DVF"].signal,
                                progress = prev_dosteps["DVF"].progress)
        end
        ...
        dosteps["virSorter"].signal = setsignal(extract_args(args, "virSorter_signal", ("use_external", "dont", "use", "ignore", "remove")),
                                        "virSorter"; cont = true, 
                                        oldsignal = prev_dosteps["virSorter"].signal,
                                        progress = prev_dosteps["virSorter"].progress)
        ...
        if (dosteps["genomad"].signal in ["ignore", "remove", "dont"] &&  dosteps["DVF"].signal in ["ignore", "remove", "dont"]
            && dosteps["virSorter2"].signal in ["ignore", "remove", "dont"] && dosteps["virSorter"].signal in ["ignore", "remove", "dont"] && dosteps["vibrant"].signal in ["ignore", "remove", "dont"] 
        ...
        touse = 0
        toignore = 0
        for p in ["genomad", "DVF", "virSorter2", "virSorter", "vibrant", "viralVerify"]
        ...
        if (touse + toignore) == 6
            ...
            println("
            You have chosen to continue an already existing DoViP project by re-runing, removing or ignoring at least one of the 6 predictors in the INITIAL PREDICTION STEP.
            The followiung rules apply: 
            ...
            if ismissing(proj.virSorter) == false
                dosteps = setstep_primary!(proj, "virSorter", dosteps, "$(pd)/$(proj.virSorter.pd)", todel; logfun = printProjSViP)
            else
                dosteps = setstep_primary!(proj, "virSorter", dosteps, missing, todel; logfun = printProjSViP)
            end
    ...
    #region predict viruses and prophages with virSorter
    virSorter = initialize_step(dosteps, "virSorter", set_ProjVirSorter, (pd, sampleName, args, dosteps["virSorter"].signal), proj.virSorter, cont)
    if dosteps["virSorter"].signal in ["use", "use_external"]
        push!(nonintegrated_viruses_to_agregate, virSorter.post_vs_nonintegrated_df)
        push!(integrated_viruses_to_agregate_df, virSorter.post_vs_integrated_df)
    end
    #endregion

    #region predict viruses and prophages with virSorter2
