...
#vs_p = "/mnt/cephfs1/projects/DeepH/Hi_C_202503_MSI/Hi_C_Alti1_interaction/virsorter1/virsorter-out/VIRSorter_global-phage-signal.csv"
#vs_out = CSV.read(vs_p, DataFrame; delim = ',', header = 2)
#prot_p = "/mnt/cephfs1/projects/DeepH/Hi_C_202503_MSI/Hi_C_Alti1_interaction/virsorter1/virsorter-out/Metric_files/VIRSorter_affi-contigs.tab"
#vs_prot = CSV.read(prot_p, DataFrame; delim = '|', header = 0)

function post_virSorter(proj::ProjVirSorter, parentD::String)
    if proj.ext_res == true
        vs_out =  CSV.read("$(proj.ext_res_D)/$(proj.vs_global_phage_sig.p)", DataFrame; delim = ',', header = 2)
        vs_prot = CSV.read("$(proj.ext_res_D)/$(proj.vs_prot_metrics)", DataFrame; delim = '|', header = 0)
    else
        vs_out =  CSV.read("$(parentD)/$(proj.vs_global_phage_sig.p)", DataFrame; delim = ',', header = 2)
        vs_prot =  CSV.read("$(parentD)/$(proj.vs_prot_metrics)", DataFrame; delim = '|', header = 0)
    end

    vs_prot_dict = Dict()

    # vs_prot
    for i in 1:nrow(vs_prot)
        if !occursin(">", vs_prot[i, :Column1])
            key = replace(vs_prot[i, :Column1], r"^VIRSorter_" => "")
            vs_prot_dict[key] = (vs_prot[i, :Column2], parse(Int64, vs_prot[i, :Column3]))
        end
    end

    # vs_out
    rename!(vs_out, Symbol("## Contig_id") => :contig_name, :Fragment => :seq_name, :Category => :category_virSorter)
    for i in nrow(vs_out):-1:1
         if occursin("##", vs_out[i, :contig_name])
            deleteat!(vs_out, i)
        end
        
        # remove VirSorter from contig name
        vs_out[i, :contig_name] = replace(vs_out[i, :contig_name], r"^VIRSorter_" => "")
        vs_out[i, :seq_name] = replace(vs_out[i, :seq_name], r"^VIRSorter_" => "")
    end

    
    # nonintegrated
    nonInt_df = deepcopy(vs_out)
    for i in nrow(nonInt_df):-1:1
        if nonInt_df[i, :contig_name] != nonInt_df[i, :seq_name] 
            deleteat!(nonInt_df, i)
        end
    end

    if !isempty(nonInt_df)
        select!(nonInt_df, [:contig_name, :category_virSorter])
        
        nonInt_df[!, :predictor_virSorter] = fill("yes", nrow(nonInt_df))
        #nonInt_df[!, :length] = fill(0, nrow(nonInt_df))
        nonInt_df[!, :shape_virSorter] = fill("linear", nrow(nonInt_df))

        for i in 1:nrow(nonInt_df)
            #m = match(r"_length_(.+?)_cov", nonInt_df[i, :contig_name])
            #nonInt_df[i, :length] = parse(Int64, m.captures[1])

            if occursin("-circular", nonInt_df[i, :contig_name]) 
                nonInt_df[i, :shape_virSorter] = "circular"
                nonInt_df[i, :contig_name] = replace(nonInt_df[i, :contig_name], "-circular" => "")
            end
        end

        CSV.write("$(parentD)/$(proj.post_vs_nonintegrated_df.p)", nonInt_df, delim = '\t', header = true)
    end


    # integrated
    int_df = deepcopy(vs_out)
    for i in nrow(int_df):-1:1
        if int_df[i, :contig_name] == int_df[i, :seq_name]
            deleteat!(int_df, i)
        end
    end

    if !isempty(int_df)
        select!(int_df, [:contig_name, :seq_name, :category_virSorter])
        int_df[!, :predictor_virSorter] = fill("yes", nrow(int_df))
        int_df[!, :length] = fill(0, nrow(int_df))
        int_df[!, :shape_virSorter] = fill("linear", nrow(int_df))
        int_df[!, :provirus_start] = fill(0, nrow(int_df))
        int_df[!, :provirus_end] = fill(0, nrow(int_df))

        for i in 1:nrow(int_df)

            m2 = match(r"-gene_(.+?)-gene_(.+?)$", int_df[i, :seq_name])
            gene_start_name = "$(int_df[i, :contig_name])-gene_$(parse(Int64, m2.captures[1]))"
            gene_end_name = "$(int_df[i, :contig_name])-gene_$(parse(Int64, m2.captures[2]))"

            int_df[i, :provirus_start] = vs_prot_dict[gene_start_name][1]
            int_df[i, :provirus_end] = vs_prot_dict[gene_end_name][2]

            int_df[i, :length] = int_df[i, :provirus_end] - int_df[i, :provirus_start] + 1

            if occursin("-circular", int_df[i, :contig_name]) 
                int_df[i, :shape_virSorter] = "circular"
                int_df[i, :contig_name] = replace(int_df[i, :contig_name], "-circular" => "")
            end

        end

        CSV.write("$(parentD)/$(proj.post_vs_integrated_df.p)", int_df, delim = '\t', header = true)
    end
...
function group_proviruses!(df::DataFrame, predictors::Vector{Symbol}, merge_circ_proph::Bool)
    ...
    newdf[!, :shape_virSorter] = Vector{Union{Missing, String}}(missing, nrow(newdf))
    ...
    for i in 1:length(ggdf)
        for j in 1:nrow(newdf)
            if ggdf[i][1, :provir] == newdf[j, :provirus_name]
                ...
                if "shape_virSorter" in names(ggdf[i])
                    newdf[j, :shape_virSorter] = find_consensus(ggdf[i][!, :shape_virSorter])
                end
                    ...
                    if (gn[lastrow, :contig_trimmed_end] - gn[lastrow, :provirus_end]) <= 1 
                        ...
                        if gn[1, :provirus_start] <= 1
                            ...
                            if (spaceR + spaceL) <= 2
                                ...
                                if :predictor_virSorter in predictors
                                    predictor_virSorter = find_consensus([gn[lastrow, :predictor_virSorter], gn[1, :predictor_virSorter]])
                                    shape_vs1 = find_consensus([gn[lastrow, :shape_virSorter], gn[1, :shape_virSorter]])
                                else
                                    predictor_virSorter = missing
                                    shape_vs1 = missing
                                end
                                ...
                                push!(anewdf, 
                                        ...
                                        predictor_virSorter = predictor_virSorter,
